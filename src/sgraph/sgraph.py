...
class SGraph:
    ...
    def to_xml(self, fname, stdout=True):
        ...
        def dump_node(c, elem_to_num, reclevel):
            sorted_attrs = sorted(filter(lambda x: not x[0].startswith('_tmp_attr_'),
                                         c.attrs.items()))
            ...
            if c in elem_to_num:
                ...
                f.write('<e ' + x + ' n="' + enc_xml_a_v(c.name) + '" ' + nattrs + '>\n')
            else:
                f.write(current_indent)
            ...
            for ealist, deptype, ea_attrs in groups:
                ...
                for ea in ealist:
                    if ea.toElement in elem_to_num:
                        ...
                        sys.stderr.write('No numeric id for ' + ea.toElement.getPath() +
                                         '  dependency from ' + ea.fromElement.name + '\n')
    ...
    def calculate_model_stats_delta(prev, stats):
        ...
        if prev is not None:
            ...
            for k in a:
                if k not in b:
                    depTypeCountsDelta[k] = -a[k]
                else:
                    depTypeCountsDelta[k] = b[k] - a[k]
            for k in b:
                if k not in a:
                    depTypeCountsDelta[k] = b[k]

            delta = (stats[0] - prev[0], stats[1] - prev[1], depTypeCountsDelta, stats[3] - prev[3])
        return delta
